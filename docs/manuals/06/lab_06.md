...
Welcome back to CS2100DE! We're nearing the final destination. We've built the critical components of our CPU, namely the instruction decoder and the immediate extension unit. Now, it's time to wire everything up and get ready to see it working. By the end of this lab, we should be able to assemble all the components of our CPU together, and get it ready for the final task next week, when we will see the CPU run real programs. 
...
Over the past few weeks, we've built the main components of our CPU. Let's explore the functions that these components perform inside our CPU in a bit more detail:
...
3. We are also given the `ALU` module. It takes two inputs:
...
Finally, we will be filling out the `RISCV_MMC.sv` file. (2100, our course code, is MMC in Roman numerals.) This is our actual CPU. For today, we can just focus on connecting everything here. Next time, we will have the rest of the scaffolding needed to make the CPU tick, and we will be able to write and run our own programs on it. 
...
This week, we will not be simulating the code. In the coming weeks, we will have a fairly complex framework to run programs and extensively debug our design. 

As usual, here is an unordered collection of tips for writing the code:

* The ALU's interpretation of the `control` signal may not be consistent with what you have designed in your decoder. Remember to fix it on either of the two sides in order to make it work correctly. 
...
Nice work! We've built our CPU, and we're ready to see it in action soon. 
